...
2. **Evaluate.** The Kyverno CLI applies every generated policy to the manifests in `samples/<sample>/`. No cluster is involved.
...
5. **Check.** oscal-cli validates the results. `check_verdicts.py` compares every rule's verdict with `samples/expected.json`, the test oracle: every rule must pass on the compliant set and fail on the non-compliant set.
6. **Summarize.** `summary.py` writes `results/<sample>/summary.md`.
...
|---|---|---|
| CM-6 Configuration settings | The Pod Security Standards restricted profile as upstream Kyverno composes it: 11 baseline policies and 6 restricted ones | The mapping's claim is "Pod Security restricted profile"; only the whole profile passing supports it |
...
1. Render your manifests first: Helm charts and Kustomize overlays must be expanded to plain YAML.
...
The results land in `results/mine/`. `samples/expected.json` has no entry for your directory, so the verdicts are reported as not checked rather than compared. The summary is evidence about those manifests, which is closer to your system than the samples are, but it is still not an assessment of a running cluster.
